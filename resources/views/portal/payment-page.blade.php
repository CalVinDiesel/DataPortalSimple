...
<html lang="en" class="layout-navbar-fixed layout-wide" dir="ltr" data-skin="default" data-assets-path="{{ asset('assets/') }}/" data-template="front-pages" data-bs-theme="light" katalonextensionid="ljdobmomdgdljniojadhoplhkpialdid"><head>
    ...
    <meta name="robots" content="noindex, nofollow">
      ...
      <title>Reload Tokens | 3DHub Data Portal</title>
  ...
  <link rel="stylesheet" href="{{ asset('assets/') }}/vendor/css/pages/front-page-payment.css">
    ...
    <style>
      ...
      #navUploadCollapsePayment .nav-link:hover { color: var(--bs-primary); }
    </style>
...
.layout-page {
  padding-top: 78.82500457763672px !important;
}
.content-wrapper {
        ...
        fetch(AUTH_API + '/api/auth/me', { credentials: 'include' }).then(function(r) { return r.json(); }).then(function(d) {
          ...
          if (!d.loggedIn || (d.role !== 'registered' && d.role !== 'trusted' && d.role !== 'admin')) {
            ...
            function showPaymentAuthPrompt() {
              var promptEl = document.getElementById('payment-signin-prompt');
              var linkEl = document.getElementById('payment-signin-link');
              if (promptEl) promptEl.classList.remove('d-none');
              if (linkEl && window.__landingUrl) linkEl.href = window.__landingUrl;
              var proceedBtn = document.getElementById('topup-proceed-btn');
              if (proceedBtn) { proceedBtn.disabled = true; proceedBtn.title = 'Sign in to reload tokens.'; }
            }
            if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', showPaymentAuthPrompt);
            else showPaymentAuthPrompt();
            return;
          }
          window.__authRole = d.role;
          var navLoginWrap = document.getElementById('navLoginWrap');
          var navUserWrap = document.getElementById('navUserWrap');
    ...
    <g id="g-app-brand" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
      ...
      <div class="collapse navbar-collapse landing-nav-menu" id="navbarSupportedContent">
        ...
        <ul class="navbar-nav me-auto">
          ...
          <li class="nav-item d-lg-none navUpload-mobile" style="display:none;">
            <a class="nav-link fw-medium dropdown-toggle" href="javascript:void(0);" data-bs-toggle="collapse" data-bs-target="#navUploadCollapsePayment" aria-expanded="false" aria-controls="navUploadCollapsePayment">
              Upload
            </a>
      ...
      <ul class="navbar-nav flex-row align-items-center ms-auto">
          ...
          <li class="nav-item dropdown-style-switcher dropdown me-2 me-xl-0">
            <a class="nav-link dropdown-toggle hide-arrow" id="nav-theme" href="javascript:void(0);" data-bs-toggle="dropdown" aria-label="Toggle theme (light)">
              <i class="bx-sun icon-base bx icon-lg theme-icon-active"></i>
        ...
        <li id="navUserWrap" style="display:none;">
          <a href="user-profile.html" class="navbar-text text-body me-2 d-none d-md-inline text-decoration-none fw-medium" id="navUserEmail"></a>
          <a href="subscriber-{{ route('login') }}" id="navSubscriberBtn" class="btn btn-outline-primary me-2" style="display:none;"><span class="d-none d-md-inline">Subscribe</span><span class="d-md-none">Subscribe</span></a>
      ...
      <div class="card px-3">
        <div class="row">
          <div class="col-lg-7 card-body border-end p-md-8">
            <h4 class="mb-2">Reload Tokens</h4>
            <p class="mb-4">Top up your 3DHub token balance. 1 token = RM2. Use tokens for raw data uploads and 3D model purchases.</p>
            <div id="token-topup-form">
              <div class="mb-4">
                <label class="form-label">Current balance</label>
                <p class="h5 mb-0" id="wallet-balance">0.00 tokens</p>
                <p class="small text-body-secondary mb-0">Rate: RM <span id="wallet-rate">2</span> per token</p>
              </div>
              <div class="mb-4">
                <label class="form-label" for="topup-tokens">Number of tokens</label>
                <input type="number" id="topup-tokens" class="form-control" min="1" step="1" value="10" placeholder="e.g. 10">
                <p class="small text-body-secondary mt-1">Amount: RM <span id="topup-myr">20.00</span></p>
              </div>
              <div id="payment-element-wrap" class="d-none">
                <label class="form-label">Payment details</label>
                <div id="payment-element"></div>
                <button type="button" class="btn btn-primary mt-3" id="pay-btn">Pay RM <span id="pay-amount">0</span></button>
                <button type="button" class="btn btn-label-secondary mt-3 ms-2" id="pay-cancel-btn">Cancel</button>
              </div>
              <button type="button" class="btn btn-primary" id="topup-proceed-btn">Proceed to payment</button>
            </div>
            <div id="token-topup-error" class="alert alert-danger mt-3 d-none" role="alert"></div>
            <div id="token-topup-success" class="alert alert-success mt-3 d-none" role="alert">Payment successful. Your balance will update shortly.</div>
          </div>
          <div class="col-lg-5 card-body p-md-8">
            <h4 class="mb-2">Summary</h4>
            <p class="mb-4">Tokens are used for uploading raw data for 3D processing and for purchasing 3D models from the map.</p>
            <ul class="list-unstyled">
              <li class="d-flex align-items-center mb-2"><span class="badge badge-center rounded-pill bg-label-primary p-0 me-2"><i class="bx bx-check"></i></span> 1 token = RM2</li>
              <li class="d-flex align-items-center mb-2"><span class="badge badge-center rounded-pill bg-label-primary p-0 me-2"><i class="bx bx-check"></i></span> Wallet is created automatically</li>
              <li class="d-flex align-items-center mb-2"><span class="badge badge-center rounded-pill bg-label-primary p-0 me-2"><i class="bx bx-check"></i></span> Secure payment via Stripe</li>
            </ul>
          </div>
        </div>
      </div>
...
<!-- Footer: End -->
  ...
  <script>
    (function () {
      ...
      var stripe = null;
      var elements = null;
      var paymentElement = null; // ✅ Track the individual element separately
  
      function showError(msg) {
        var el = document.getElementById('token-topup-error');
        if (el) { el.textContent = msg || ''; el.classList.remove('d-none'); }
        document.getElementById('token-topup-success').classList.add('d-none');
      }
      function showSuccess(msg) {
        var el = document.getElementById('token-topup-success');
        if (el) { el.textContent = msg || 'Payment successful. Your balance will update shortly.'; el.classList.remove('d-none'); }
        document.getElementById('token-topup-error').classList.add('d-none');
      }
      ...
      function updateMyr() {
        var n = parseInt(document.getElementById('topup-tokens').value, 10) || 0;
        document.getElementById('topup-myr').textContent = (n * rate).toFixed(2);
      }
      ...
      function loadWallet() {
        fetch(API + '/api/token/wallet', { credentials: 'include' })
          ...
          .then(function (d) {
            if (d.success) {
              document.getElementById('wallet-balance').textContent = Number(d.balance).toFixed(2) + ' tokens';
              if (d.tokenMyrRate) rate = d.tokenMyrRate;
              document.getElementById('wallet-rate').textContent = rate;
              updateMyr();
              if (d.stripePublishableKey) {
                stripe = window.Stripe(d.stripePublishableKey);
              }
            }
          })
          .catch(function () { document.getElementById('wallet-balance').textContent = '0.00 tokens'; });
      }
  
      document.getElementById('topup-tokens').addEventListener('input', updateMyr);
  
      document.getElementById('topup-proceed-btn').addEventListener('click', function () {
        var tokens = parseInt(document.getElementById('topup-tokens').value, 10);
        if (!tokens || tokens < 1) { showError('Enter at least 1 token.'); return; }
        hideMessages();
        var btn = this;
        btn.disabled = true;
        fetch(API + '/api/token/topup-intent', {
          method: 'POST',
          credentials: 'include',
          ...
          .then(function (d) {
            btn.disabled = false;
            if (!d.success) { showError(d.message || 'Failed to create payment.'); return; }
            if (!stripe || !d.clientSecret) { showError('Stripe is not configured. Please set STRIPE_PUBLISHABLE_KEY and STRIPE_SECRET_KEY on the server.'); return; }
            currentClientSecret = d.clientSecret;
            document.getElementById('pay-amount').textContent = (d.amountMyr || (tokens * rate)).toFixed(2);
  
            var wrap = document.getElementById('payment-element-wrap');
            wrap.classList.remove('d-none');
            document.getElementById('topup-proceed-btn').classList.add('d-none');
  
            // ✅ Destroy previous payment element (not elements object) before re-creating
            if (paymentElement) {
            ...
            elements = stripe.elements({ clientSecret: d.clientSecret, appearance: appearance });
            paymentElement = elements.create('payment');
            document.getElementById('payment-element').innerHTML = '';
            paymentElement.mount('#payment-element');
          })
          .catch(function (e) { btn.disabled = false; showError(e.message || 'Network error.'); });
      });
  
      document.getElementById('pay-cancel-btn').addEventListener('click', function () {
        document.getElementById('payment-element-wrap').classList.add('d-none');
        document.getElementById('topup-proceed-btn').classList.remove('d-none');
        document.getElementById('payment-element').innerHTML = '';
        currentClientSecret = null;
  
        // ✅ Destroy the individual payment element, not the elements object
        if (paymentElement) {
          paymentElement.destroy();
          paymentElement = null;
        }
      ...
      document.getElementById('pay-btn').addEventListener('click', async function () {
        if (!currentClientSecret || !stripe || !elements) return;
        var btn = this;
        btn.disabled = true;
  
        // ✅ Step 1: Call elements.submit() FIRST before confirmPayment
        var submitResult = await elements.submit();
        if (submitResult.error) {
          showError(submitResult.error.message || 'Validation failed.');
          btn.disabled = false;
          return;
        }
...
<div id="template-customizer" class="card rounded-0" style="visibility: hidden"> <a href="javascript:void(0)" class="template-customizer-open-btn" tabindex="-1"></a> <div class="p-6 m-0 lh-sm border-bottom template-customizer-header position-relative py-4"> <h6 class="template-customizer-t-panel_header mb-1">Template Customizer</h6> <p class="template-customizer-t-panel_sub_header mb-0 small">Customize and preview in real time</p> <div class="d-flex align-items-center gap-2 position-absolute end-0 top-0 mt-6 me-5"> <a href="javascript:void(0)" class="template-customizer-reset-btn text-heading" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Reset Customizer"><i class="icon-base bx bx-refresh icon-24px"></i><span class="badge rounded-pill bg-danger badge-dot badge-notifications"></span></a> <a href="javascript:void(0)" class="template-customizer-close-btn fw-light text-heading" tabindex="-1"> <i class="icon-base bx bx-x icon-lg"></i> </a> </div> </div> <div class="template-customizer-inner pt-6"> <div class="template-customizer-theming"> <h5 class="m-0 px-6 pb-6"> <span class="template-customizer-t-theming_header bg-label-primary rounded-1 py-1 px-3 small">Theming</span> </h5> <div class="m-0 px-6 pb-6 template-customizer-color w-100"> <label for="customizerColor" class="form-label d-block template-customizer-t-color_label mb-2">Primary Color</label> <div class="row template-customizer-colors-options"><div class="form-check custom-option custom-option-icon mb-0 checked">
          <label class="form-check-label custom-option-content p-0" for="colorRadioIconprimary">
            <span class="custom-option-body mb-0 scaleX-n1-rtl" style="background-color: #696cff;"></span>
          </label>
          <input name="colorRadioIcon" class="form-check-input d-none" type="radio" value="#696cff" data-color="#696cff" id="colorRadioIconprimary" checked="checked">
        ...
        <button type="button" class="pcr-button bx bxs-color-fill" role="button" aria-label="toggle color picker dialog" style="transition: none; --pcr-color: rgba(255, 73, 97, 1);"></button>
      ...
      </div></div> </div>   <hr class="m-0 px-6 my-6"> </div> <div class="template-customizer-layout"> <h5 class="m-0 px-6 pb-6"> <span class="template-customizer-t-layout_header bg-label-primary rounded-2 py-1 px-3 small">Layout</span> </h5>     <div class="m-0 px-6 pb-6 template-customizer-directions w-100"> <label for="customizerDirection" class="form-label template-customizer-t-direction_label mb-2">Direction</label> <div class="row px-1 template-customizer-directions-options"><div class="col-4 px-2">
      ...
      </div></div> </div> </div> </div> </div><script type="text/javascript" id="" charset="">console.log("TS:GTM Worked");</script><div style="display: none; visibility: hidden;">
...
</div><div class="pcr-app " data-theme="nano" aria-label="color picker dialog" role="window" style="left: 0px; top: 8px;">
